...
channel_names = {'CFP'};
ringcalc = 0;
num_frames = 400; % start from frame 1, until frame num_frames

addpath('Functions/');
image_dir = 'Z:/projects/tracking_code/submission2/BGU_RPE/Raw/';

%% define mask directory, load cmos offset, define dir of bias
base_output_dir = 'Z:/projects/tracking_code/submission2/BGU_RPE/mingyu/';
cmosoffset_dir = '';
bias_dir = '';
...
% read the files
raw = imread([image_dir, 'M20150416_w2CFP_s102_t001.tif']);
raw = cat(3, raw, imread([image_dir, 'M20150416_w2CFP_s102_t002.tif']));
raw = cat(3, raw, imread([image_dir, 'M20150416_w2CFP_s102_t003.tif']));
...
for i=1:num_frames
    ...
    for j=1:num_channels
        raw{j} = max((double(imread([image_dir, 'M20150416_w2CFP_s102_t', sprintf('%03d', f), '.tif'])) - cmos)./bias{j}, 1);
    end
